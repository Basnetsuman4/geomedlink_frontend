...
export const UploadPost = () => {
  ...
  return (
    <View style={styles.container}>
      ...
      <ScrollView>
      <View style={{ padding:4,borderWidth:1,borderColor:"#000",marginHorizontal:8,marginBottom:8}}>
        <View style={{display:'flex',flexDirection:'row',justifyContent:'space-between'}}>
        <Text style={{fontSize:18,fontWeight:'bold',marginVertical:8}}>Create Post</Text>
        <Button mode="contained" disabled={!canPost()} onPress={upload} style={{width:'25%',marginVertical:4,borderRadius:8}}>
          Post
        </Button>
        </View>
        <TextInput
        ...
        <View style={styles.thirdView}>
          {selectedImage && (
            <View >
           <Icon  style={{display:'flex',alignSelf:'flex-end'}}
                onPress={() => {
                  setSelectedImage('');
                }}
                name="cross"
          ...
          <View  >
            ...
            {!selectedImage ? (
              <Row>
                <View style={{width:'100%',gap:10,display:'flex',flexDirection:'row',alignItems:'center'}}>
                  <Text>Upload Attachment </Text>
                  <Button mode="elevated" onPress={pickImage}>Select File</Button>
                </View>
              </Row>
            ) : (''
            )}
      ...
      <View style={{ padding:4,borderWidth:1,borderColor:"#000",marginHorizontal:8}}>
      <AddEvent/>
      </View>
      </ScrollView>
    </View>
